...
def split_dataset(df: DataFrame, args: list[str]):
    ...
    if len(args) <= 2:
        ...
        training_data, test_data = match_specified_split(df, specified_split)
    return training_data, test_data

# do train/test split based on dataframe and given keyword
def match_specified_split(df: DataFrame, keyword: str):
    match keyword:
        case "pbp":
            print("Commencing PBP isolation split as specified")
            training_data, test_data = pbp_isolation_split(df)
        case _:
...
def isolate_attacks(df: DataFrame, search_strings: list):
    # create empty dataframe with same column and column types as df argument
    ...
    for search_string in search_strings:
        ...
        attack_df = spark.createDataFrame(data=attack_rows, schema=isolated_df.schema)  # create df from list of rows
    ...
    if isolated_df.count() == 0:
        ...
        # remaining_df (to be used as training df) contains all rows in df that are not in the isolated df

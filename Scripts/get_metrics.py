...
def get_shap():
    ...
    abs_preds_dir_path = get_predictions_data_directory(
        model_id)  # try to load predictions data
    pred_df = spark.read.csv(abs_preds_dir_path, header=True,
                             inferSchema=True)
    ...
    portscan_shap_values.write.option("header", True).mode("overwrite").csv(
        (cwd + "/Data/Results/portscan_shap" + model_id))
    patator_shap_values.write.option("header", True).mode("overwrite").csv(
        (cwd + "/Data/Results/patator_shap" + model_id))
...
def plot_shap_force(shap_values, explainer, df, indexes):
    for index in indexes:
        shap.force_plot(explainer.expected_value[0], shap_values[index], df.iloc[index], show=True)
